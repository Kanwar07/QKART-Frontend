...
const Checkout = () => {
  ...
  useEffect(() => {
    if(token) {
      ...
      enqueueSnackbar("You must be logged in to access checkout page",{varient:"info"});
      history.push('/login');
    }
  }, [token])
  /**
  ...
  const validateRequest = (items, addresses) => {
    if(localStorage.getItem('balance') < getTotalCartValue(items)) {
      enqueueSnackbar('You do not have enough balance in your wallet for this purchase', {varient:"warning",});
      return false;
    }
    if(!addresses.all.length) {
      enqueueSnackbar("Please add a new address before proceeding", {varient:"warning",});
      return false;
    }
    if(!addresses.selected.length) {
      enqueueSnackbar("Please select one shipping address to proceed", {varient:"warning",});
      return false;
    }
    return true;
  };
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      enqueueSnackbar("Order placed successfully", {varient:"success"});
      ...
      if(e.response.status === 400) {
        enqueueSnackbar("Wallet balance not sufficient to place order", {varient:"error"});
      }
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
    ...
    <>
      ...
      <Grid container>
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          <Cart isReadOnly hasCheckoutButton={false} products={products} items={items}> 
          </Cart>
        </Grid>
      </Grid>
      <Footer />
